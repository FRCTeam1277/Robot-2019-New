...
public class Drive {
	...
	public static void tele() {
		double move, rotate;
		if (Math.abs(OI.joystick.getRawAxis(1)) > Math.abs(OI.xbox.getRawAxis(1))) {
			move = -OI.joystick.getRawAxis(1);
		} else {
			move = -OI.xbox.getRawAxis(1);
		}
		if (Math.abs(OI.joystick.getRawAxis(2)) > Math.abs(OI.xbox.getRawAxis(0))) {
			rotate = OI.joystick.getRawAxis(2);
		} else {
			rotate = OI.xbox.getRawAxis(0);
		}
