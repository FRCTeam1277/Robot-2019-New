...
public class OI {
	...
	public static Button aButton = new JoystickButton(xbox, 0);
	public static Button bButton = new JoystickButton(xbox, 1);
	public static Button xButton = new JoystickButton(xbox, 2);
	public static Button yButton = new JoystickButton(xbox, 3);
	public static Button lButton = new JoystickButton(xbox, 4);
	public static Button rButton = new JoystickButton(xbox, 5);
	public static Button viewButton = new JoystickButton(xbox, 6);
	public static Button menuButton = new JoystickButton(xbox, 7);
	public static Button lStick = new JoystickButton(xbox, 8);
	public static Button rStick = new JoystickButton(xbox, 9);
	...
	public OI() {
		...
		// Xbox
		yButton.whenPressed(new Release());
		menuButton.whenPressed(new Reset());
		viewButton.whenPressed(new Front());
		aButton.whenPressed(new GrabHatch());
		lButton.whenPressed(new PickUpBall());
		rButton.whenPressed(new AimCargoShip());
		xButton.whenPressed(new HomeShooter());

	}

	public static void rumble() {
		if (GrabHatch.executing) {
			xbox.setRumble(RumbleType.kLeftRumble, 0.25);
		} else {
	...
	public static void POV() {
		if (joystick.getPOV() == 0 /*|| xbox.getPOV() == 0*/) {
			Shooter.aimUp();
		}
		if (joystick.getPOV() == 180 /*|| xbox.getPOV() == 180*/) {
			Shooter.aimDown();
		}
		if (OI.joystick.getPOV() == 90 /*|| xbox.getPOV() == 90*/) {
			Hatch.hatchRight();
		}
		if (joystick.getPOV() == 270 /*|| xbox.getPOV() == 270*/) {
			Hatch.hatchLeft();
		}
		if (joystick.getPOV() == -1 /*&& xbox.getPOV() == -1*/) {
			Shooter.stopAim();
			Hatch.hatchStop();
		}
	}
